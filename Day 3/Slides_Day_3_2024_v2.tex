...
\pause
\end{itemize}
\item Not-quite natural experiments
\begin{itemize}
...
\item Assumptions
\begin{itemize}
\item \textbf{Randomization respected in implementation}
\pause
\item \textbf{Compliance with randomization} - Treatment \textit{accepted}
\pause
\begin{itemize}
\item Very rare to have full compliance!
\item Compliance is not likely to be random, creating bias
\pause
\end{itemize}
\item \textbf{No Spillovers (SUTVA)} - Treatment of one unit doesn't affect potential outcomes of other units
\pause
\begin{itemize}
\item If I receive a vaccine, that reduces the risks to everyone
\item If you are given information about a policy, you might pass it on to your family
\end{itemize}
\end{itemize}
\end{itemize}
\end{frame}
...
\item Ethical/financial constraints
\item Some treatments can't be manipulated (history)
\item Scale effects - is the effect the same when implemented by a government?
\item Hawthorne effects - people react to being part of an experiment
\item Generalizability/external validity often poor
\item No information about the mechanism
\end{itemize}
\end{itemize}
\end{frame}
...
\pause
\begin{enumerate}
\item Use an all-in-one package
\begin{itemize}
...
\item Limitations:
\begin{itemize}
\item Most treatments do not have a valid instrument
\item The exclusion restriction is not testable
\item The conclusion has narrow scope/generalizability - LATE
\item Low precision
\item No information about the mechanism
\end{itemize}
\end{itemize}
...
\item Regression Discontinuity
\begin{itemize}
\item What is the treatment assignment mechanism for passing an exam?
\pause
...
\pause
\item Units cannot precisely 'manipulate' their score and self-select to either side of the threshold
\pause
...
\begin{center}
\includegraphics[scale=1]{figure/Density-2.pdf}
\end{center}
\end{frame}

\begin{frame}
\begin{itemize}
\item The regression discontinuity analysis:
...
\pause
\begin{itemize}
\item Treatment assignment is only as-if random at the threshold
\pause
\item Our estimates are only applicable/generalizable to units close to the threshold: a \textbf{Local Average Treatment Effect}
\pause
...
\pause
\item Low generalizability - LATE
\begin{itemize}
\item What if most elections are landslides?
\end{itemize}
\pause
\item Lots of alternative specifications so no single simple test - publication bias?
\pause
\item Opportunistic regression discontinuities may not identify a useful causal effect or for a relevant group
\begin{itemize}
\item Researchers often 'look' for where they can run a regression discontinuity, even if it doesn't answer their research question
\end{itemize}
\pause
\item No information about the mechanism
\end{itemize}
...
\pause
\item There is nothing 'as-if random' here, but we are more easily able to limit the risk of omitted variables
\end{itemize}
\end{frame}
...
\frametitle{Difference-in-Differences}
\begin{itemize}
\item Difference-in-differences removes \textbf{time-invariant ('levels') confounders}
\pause
\begin{itemize}
\item Even if they are unobserved/unmeasured
\pause
...
\pause
\item Eg. Even before Brexit, the UK had falling growth while growth in the eurozone was improving due to differences in investment/productivity
\end{itemize}
\end{frame}
...
\pause
\item Time (Before and after treatment) and treatment status (treated and control) are just variables in our data
\pause
...
\begin{itemize}
\item Not a true natural experimental method 
\pause
\item Reliant on no time-varying confounders/parallel trends assumption
\pause
\item Often we have no control units - everyone gets treated at the same time
\pause
\item No information about the mechanism
\end{itemize}
\end{itemize}

...
\section{Consistent Explanation}

\begin{frame}
\frametitle{Consistent Explanation}
\begin{itemize}
\item To be good causal explanations, explanations need to be \textbf{logically consistent}
\pause
\begin{itemize}
\item How we combine evidence and reach conclusions must avoid 'cheating'
\end{itemize}
\pause

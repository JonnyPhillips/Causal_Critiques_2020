...
\pause
\begin{itemize}
\item If we obtain a receipt for the charger that shows it is high quality and only two weeks old
\end{itemize}
\pause
\item \textbf{Hoop Test}: Can reject a hypothesis but not confirm
\pause
\begin{itemize}
\item If we test the laptop with another charger and socket to make sure it works
\end{itemize}
\pause
\item \textbf{Smoking Gun Test}: Can confirm a hypothesis but not reject
\pause
\begin{itemize}
\item If we test the charger to see if it fails in another socket
\end{itemize}
\pause
\item \textbf{Doubly Decisive Test}: Can confirm a hypothesis and reject all other hypotheses
\begin{itemize}
\item If we test the charger with an entirely new socket and laptop that we have checked work
\end{itemize}
\end{enumerate}
\end{itemize}
\end{frame}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: Prefixing `UQ()` with the rlang namespace is deprecated as of rlang 0.3.0.\\\#\# Please use the non-prefixed form or `!!` instead.\\\#\# \\\#\#\ \  \# Bad:\\\#\#\ \  rlang::expr(mean(rlang::UQ(var) * 100))\\\#\# \\\#\#\ \  \# Ok:\\\#\#\ \  rlang::expr(mean(UQ(var) * 100))\\\#\# \\\#\#\ \  \# Good:\\\#\#\ \  rlang::expr(mean(!!var * 100))\\\#\# \\\#\# This warning is displayed once per session.}}\end{kframe}

{\centering \includegraphics[width=500px,height=500]{figure/unnamed-chunk-1-1} 
...
{\centering \includegraphics[width=600px,height=600]{figure/unnamed-chunk-3-1} 
